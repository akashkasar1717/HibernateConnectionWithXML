var noOfRecordsPerPage = 15;
var noOfPages = null;
var searchStringGlobal = null;
var searchTypeGlobal = null;
var isSearch = false;
var countUrl = null;
var getNextRecordsUrl = null;
var count = 0;
var verifyAction = null;
var fromDateGlobal = null;
var toDateGlobal = null;
//CR no. -2021-1019  for KDMC 
function initialize(countUrlLocal, getNextRecordsUrlLocal, verifyActionLocal) {
	countUrl = countUrlLocal;
	alert(countUrl);
	getNextRecordsUrl = getNextRecordsUrlLocal;
    alert(getNextRecordsUrl);
	verifyAction = verifyActionLocal;
	getNoOfRecords('');
}

function pagination(noOfRecords) {
	noOfPages = Math.ceil(noOfRecords * 1.0 / noOfRecordsPerPage)
	$('.pagination').bootpag({
		total : noOfPages,
		maxVisible : 10,
		leaps : true,
		firstLastUse : true,
		first : '←',
		last : '→',
		wrapClass : 'pagination',
		activeClass : 'active',
		disabledClass : 'disabled',
		nextClass : 'next',
		prevClass : 'prev',
		lastClass : 'last',
		firstClass : 'first'
	}).on("page", function(event, num) {
		if (num != 1) {
			var start = Number(num * noOfRecordsPerPage - noOfRecordsPerPage);
		} else {
			var start = 0;
		}
		getNextRecords(searchStringGlobal, searchTypeGlobal, start);
	});
}

function getNoOfRecords(searchString, searchType) {
	
	$.ajax({
		type : "POST",
		url : countUrl,
		data : {
			searchType:searchType,
			searchString : searchString,
			verifyAction : verifyAction
		}
	}).done(function(result) {
		if (result != null) {
			if (result > noOfRecordsPerPage) {
				pagination(result);
				$('.pagination').show();
			} else
				$('.pagination').hide();
			searchStringGlobal = searchString;
			searchTypeGlobal = searchType;
			isSearch = true;
			count = count + 1;
			if (count > 1)
				getNextRecords(searchString,searchType, 0);
		} else {
			searchStringGlobal = null;
			searchTypeGlobal = null;
			isSearch = false;
		}

	}).fail(function() {

	});
}

function getNextRecords(searchString,searchType, start) {
	$.ajax({
		type : "POST",
		url : getNextRecordsUrl,
		data : {
			offset : start,
			searchString : searchString,
			searchType : searchType,
			verifyAction : verifyAction
		}
	}).done(function(result) {
		$(".data").remove();
		$("#tableBody").append(result);
	}).fail(function() {

	});
}
